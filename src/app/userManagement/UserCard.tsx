...
export const UserCard: FC<UserCardProps> = memo(({ user, onEdit, isLoading = false }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-5 hover:border-[#663399]/30 transition-all duration-300">
      <div className="flex justify-between items-start mb-4 relative">
        <div className="min-w-0 flex-1">
          ...
          {formattedUser.email && (
            <a href={`mailto:${formattedUser.email}`} 
               className="text-sm text-gray-500 hover:text-[#663399] transition-colors 
                          inline-flex items-center gap-2 mt-1 max-w-[calc(100%-40px)]" // Add max-width
            >
              <Mail size={14} className="flex-shrink-0" />
              <span className="truncate block w-full overflow-hidden text-ellipsis">
        ...
        <button
          ...
          className="text-gray-400 hover:text-[#663399] transition-colors p-2 rounded-md hover:bg-[#663399]/5 flex-shrink-0 ml-2"
        >
          <Edit2 size={18} />
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <>
          {formattedUser.relationships
          .filter(relation => relation.relation === 'father')
          .map((relation, index) => (
            <span
            key={`father-${index}`}
            ...
            <span className="font-medium">
              ...
              ? `${capitalizeFirst(relation.firstName)} ${capitalizeFirst(relation.lastName)}`
              : 'Unnamed'
              }
            </span>
            <span className="ml-1 text-gray-500">(Father)</span>
            </span>
          ))}

          {formattedUser.relationships
          .filter(relation => relation.relation === 'son' || relation.relation === 'daughter')
          .map((relation, index) => (
            <span
            key={`child-${index}`}
            className="inline-flex items-center px-3 py-1 rounded-full text-xs
        ...
        <span className="text-sm text-gray-500">No family members mapped</span>
        )}
      </div>
      </div>
    </div>
